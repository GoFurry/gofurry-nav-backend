...
func (dao *navPageDao) GetSiteList() (res []models.GfnSite, err common.GFError) {
	db := dao.Gm.Table(models.TableNameGfnSite)
	db.Order("id ASC")
	db.Find(&res)
...
// 按 id 序返回指定的金句
func (dao navPageDao) GetSayingByOrder(index int) (*models.GfnSaying, common.GFError) {
	var res models.GfnSaying
	db := dao.Gm.Table("gfn_saying").Order("id")
	db.Offset(index).Limit(1)
	db.First(&res)
