...
// @Summary 提供背景随机图片
// @Schemes
// @Description 提供背景随机图片, type= resized 或 normal 默认 normal
// @Tags Nav-search
// @Accept json
// @Produce json
// @Param type query string true "图片类型"
// @Success 200 {object} common.ResultData
// @Router /api/nav/page/header/getImage [Get]
func (api *navPageApi) GetImage(c *fiber.Ctx) error {
	t := c.Query("type", "normal")
	imagePath, err := service.GetNavPageService().GetImageService(c, t)
	if err != nil {
		return common.NewResponse(c).Error(err)
	}
